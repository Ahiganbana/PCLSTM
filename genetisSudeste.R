########PREPROCESSING###########
mydata <- read.csv("D:/makaleler/deepLearnTuning/derivativeFreeEnsemble/dataSet/19911_25870_compressed_sudeste.csv/sudeste.csv",nrows=2000)
mydata[is.na(mydata)] <- 0
write.csv(mydata,file="sudeste.csv")
##############################
library(nloptr)
library("mxnet")
library("abind")
...
if(toplam1>toplam2){sonuc <- mod(toplam1,toplam2)}
if(toplam1<toplam2){ sonuc <- mod(toplam2,toplam1)}
return(sonuc)
}
#######en iyi dropout bul###########################
library("GA")
f <- function(x)  f1(x)
GA <- ga(type = "real-valued", fitness = f,maxiter=10, lower = 0.1, upper = 0.9)
summary(GA)
plot(GA)
##################################
