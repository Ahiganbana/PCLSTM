...
pred_length <- 100
predicted <- numeric()
data <- mx.nd.array(trainX[, , 10, drop = F])
label <- mx.nd.array(trainY[, 10, drop = F])
...
for (i in 1:2) {
    data <- mx.nd.array(trainX[, i, 10, drop = F])
    label <- mx.nd.array(trainY[i, 10, drop = F])
    infer.data <- mx.io.arrayiter(data = data, 
                                  label = label, 
                                  batch.size = 1, 
                                  shuffle = FALSE)
